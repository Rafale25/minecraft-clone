...
inline Blueprint createBlueprintFromSelection(const glm::ivec3 min, const glm::ivec3 max) {
    std::vector<std::tuple<glm::ivec3, BlockType>> blocks;
...
inline void saveBlueprintToFile(const Blueprint& blueprint, const std::string& name)
{
    std::ofstream file;
    file.open(name);
...
inline Blueprint createBlueprintFromFile(const std::string& name) {
    Blueprint bp = {};
    std::ifstream file;
    std::string str = "";
    std::smatch m = {};
